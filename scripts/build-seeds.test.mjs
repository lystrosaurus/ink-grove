...
async function fixture(t) {
  ...
  const seed = {
    ...
    cover: '/seeds/small-step/cover.svg',
    related: [],
  ...
  const catalog = {
    schemaVersion: 1,
    seeds: [seed],
    layers: [{ id: 4, name: '山谷', goal: '学习', description: '一步一步试', color: '#46788a' }],
  }
  await writeFile(
    resolve(workspace, 'content/seeds/small-step/manifest.json'),
    JSON.stringify(seed),
  )
  await writeFile(
    resolve(workspace, 'content/seeds/small-step/index.html'),
    '<!doctype html><html lang="zh-CN"><body><button>试试</button></body></html>',
  )
  await writeFile(
    resolve(workspace, 'content/seeds/small-step/cover.svg'),
    '<svg xmlns="http://www.w3.org/2000/svg"/>',
  )
  await writeFile(
    resolve(workspace, 'content/seed-catalog.json'),
    JSON.stringify({ ...catalog, seeds: [{ id: 'small-step', title: '过时缓存' }] }),
  )
...
test('Seed manifests are authoritative and the build only adds the shared reading bridge to generated HTML', async (t) => {
  ...
  const generated = JSON.parse(
    await readFile(resolve(workspace, 'app/src/generated/seed-catalog.json'), 'utf8'),
  )
  assert.equal(generated.seeds[0].title, '小步')
  assert.equal(generated.seeds[0].parentPrompt, '一起观察结果。')
  assert.equal(Object.hasOwn(generated.seeds[0], 'age'), false)
  assert.equal(await readFile(sourcePath, 'utf8'), original)
  assert.match(
    await readFile(resolve(workspace, 'app/public/seeds/small-step/index.html'), 'utf8'),
    /data-ink-grove-bridge/,
  )
  assert.equal(
    await readFile(resolve(workspace, 'app/public/artifacts/keep.txt'), 'utf8'),
    'adult work',
  )
})

test('Seed validation rejects invalid identities, stale age categories, regions, prompts and dangling relationships', async (t) => {
  ...
  const cases = [
    ...
    [
      (c) => c.seeds.push({ ...structuredClone(c.seeds[0]), id: 'another-seed' }),
      /duplicate.*slug/i,
    ],
    [(c) => (c.seeds[0].slug = '../escape'), /slug/i],
    [(c) => (c.seeds[0].layer = 3), /layer/i],
    [(c) => (c.seeds[0].age = ['6-8', '9-11', '12-15']), /age/i],
    [(c) => (c.seeds[0].age = []), /age/i],
...
test('a stale age category in the index is rejected even when its manifest has no age field', async (t) => {
  const { buildSeeds } = await builder()
  const { workspace, catalog } = await fixture(t)
  await writeFile(
    resolve(workspace, 'content/seed-catalog.json'),
    JSON.stringify({ ...catalog, seeds: [{ id: 'small-step', age: ['6-8'] }] }),
  )
  await assert.rejects(buildSeeds({ workspace, checkOnly: true }), /age/i)
})
...
test('the Seed index cannot silently omit a content directory', async (t) => {
  ...
  await writeFile(
    resolve(root, 'forgotten-seed/index.html'),
    '<html><body>Unindexed work</body></html>',
  )
...
test('Seed resource paths cannot traverse, use a different seed or resolve outside their directory', async (t) => {
  ...
  for (const src of [
    '/seeds/small-step/../index.html',
    '/seeds/other/index.html',
    '/seeds/small-step/%2e%2e/index.html',
    'https://example.com/a.html',
    '/seeds/small-step/index.html?x=1',
  ]) {
    const changed = structuredClone(catalog)
...
test('Check-only validates without changing outputs and a failed build preserves existing outputs', async (t) => {
  ...
  assert.equal(
    await readFile(resolve(workspace, 'app/public/seeds/keep.txt'), 'utf8'),
    'previous content',
  )
