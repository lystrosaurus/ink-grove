...
const AGES = new Set(['6-8', '9-11', '12-15'])
const MVP_LAYERS = new Set([1, 2, 4, 5])
const validSlug = (value) => typeof value === 'string' && value.length <= 80 && SLUG.test(value)
const nonempty = (value) => typeof value === 'string' && value.trim().length > 0
const strings = (value, minimum = 0) => Array.isArray(value) && value.length >= minimum && value.every(nonempty) && new Set(value).size === value.length
...
async function unredirected(workspace, target, allowMissing = false) {
  ...
  for (const part of parts) {
    ...
    try {
      requireValue(!(await lstat(current)).isSymbolicLink(), `Refusing redirected symbolic path: ${current}`)
    } catch (error) {
  ...
  requireValue(actualTarget === actualBase || contained(actualBase, actualTarget), `Redirected path: ${target}`)
}

async function localResource(url, id, seedRoot) {
  requireValue(typeof url === 'string' && url.startsWith(`/seeds/${id}/`) && !/[\\?#%]/.test(url), `Expected local seed path: ${id}: ${url}`)
  const parts = url.slice('/seeds/'.length).split('/')
  requireValue(parts.every((part) => part && part !== '.' && part !== '..'), `Expected local seed path: ${id}: ${url}`)
...
export async function validateSeedCatalog(catalog, root = resolve(WORKSPACE, 'content/seeds')) {
  ...
  requireValue(Array.isArray(catalog.seeds) && catalog.seeds.length > 0, 'Seed catalog must contain seeds')
  requireValue(Array.isArray(catalog.layers) && catalog.layers.length > 0, 'Seed catalog must contain layers')
  const layers = new Set()
  for (const layer of catalog.layers) {
    requireValue(MVP_LAYERS.has(layer.id) && !layers.has(layer.id), `Invalid or duplicate layer: ${layer.id}`)
    for (const field of ['name', 'goal', 'description', 'color']) requireValue(nonempty(layer[field]), `Missing layer ${field}: ${layer.id}`)
  ...
  for (const seed of catalog.seeds) {
    requireValue(validSlug(seed?.id) && !ids.has(seed.id), `Invalid or duplicate seed id: ${seed?.id}`)
    requireValue(validSlug(seed.slug) && !slugs.has(seed.slug), `Invalid or duplicate seed slug: ${seed.slug}`)
    ...
    requireValue(strings(seed.age, 1) && seed.age.every((age) => AGES.has(age)), `Invalid seed age: ${seed.id}`)
    for (const field of ['title', 'subtitle', 'goal', 'type', 'parentPrompt', 'realLifePrompt']) requireValue(nonempty(seed[field]), `Missing ${field}: ${seed.id}`)
    for (const field of ['questions', 'skills']) requireValue(strings(seed[field], 1), `Invalid ${field}: ${seed.id}`)
    requireValue(Number.isInteger(seed.duration) && seed.duration > 0 && seed.duration <= 60, `Invalid duration: ${seed.id}`)
    ...
    requireValue(['ai', 'human', 'hybrid'].includes(seed.provenance?.generatedBy) && strings(seed.provenance.sources, 1) && strings(seed.provenance.derivedFrom), `Invalid provenance: ${seed.id}`)
    ids.add(seed.id)
    slugs.add(seed.slug)
  }
  for (const seed of catalog.seeds) {
    for (const id of seed.related) requireValue(id !== seed.id && ids.has(id), `Unknown or self related seed: ${seed.id} → ${id}`)
    for (const id of seed.provenance.derivedFrom) requireValue(id !== seed.id && ids.has(id), `Unknown or self derived seed: ${seed.id} → ${id}`)
    await localResource(seed.artifact.src, seed.id, seedRoot)
    await localResource(seed.cover, seed.id, seedRoot)
    requireValue(/\.html$/.test(seed.artifact.src) && /\.svg$/.test(seed.cover), `Expected HTML seed and SVG cover: ${seed.id}`)
    for (const source of seed.provenance.sources) {
      ...
      requireValue(parts.every((part) => part && part !== '.' && part !== '..') && !/[\\?#%:]/.test(source.slice(5)), `Invalid source path: ${source}`)
...
export async function buildSeeds({ checkOnly = false, workspace = WORKSPACE } = {}) {
  ...
  for (const entry of await readdir(seedRoot, { withFileTypes: true })) {
    requireValue(!entry.isSymbolicLink(), `Refusing redirected symbolic seed directory: ${entry.name}`)
    if (entry.isDirectory()) requireValue(indexedIds.has(entry.name), `Unindexed seed directory: ${entry.name}`)
  }
  ...
  requireValue(contained(publicRoot, output) && relative(publicRoot, output) === 'seeds', 'Unsafe Seed output directory')
  ...
  for (const seed of seeds) {
    await cp(resolve(seedRoot, seed.id), resolve(output, seed.id), { recursive: true, dereference: false })
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  buildSeeds({ checkOnly }).then((catalog) => console.log(`${checkOnly ? 'Validated' : 'Built'} ${catalog.seeds.length} seeds in ${catalog.layers.length} regions.`)).catch((error) => {
