...
一个可在本机使用、也可通过 CloudBase 静态分发的视觉知识花园。发现作品，沉浸阅读，沿概念建立连接，再把新理解种回花园。

**1.1.0 · 从判断到行动。**本版汇集 31 件作品、7 个精选集合与 35 个概念，新增十一份书籍素材和交互贯通作品《清醒行动实验室》，进一步连接判断、投资、管理效能与当下觉察。阅读、组织与个人创作数据仍保存在浏览器；AI 接口保留且默认关闭。发行内容见 [CHANGELOG](CHANGELOG.md)，仓库协作约定见 [AGENTS.md](AGENTS.md)。
...
1.1.0 已部署到 [Ink Grove 在线花园](https://ink-d0gvorjko99e99e4d-1303038884.tcloudbaseapp.com/)。31 件作品已上线，201 个产物逐一校验通过；测试与公网检查见 [发布验收记录](artifacts/verification.json)。默认测试域名首次访问会显示平台提示，等待倒计时后点击「确定访问」即可继续。
...
线上分发不会同步个人数据。本机、默认域名或未来自定义域名各自拥有独立的浏览器存储，可通过花园 JSON 导出、导入迁移。
...
`materials/` 中的 27 件原始 HTML 保持原字节，内容目录默认直接复制原稿。五件展示副本记录了明确修复：《系统之美》的层级遗漏、《贯通思考 II》的移动端矩阵标签，以及本版《控制论》的手机闭环图、《管理决策中的判断》的窄屏签名区和《聪明的投资者》的手机资产示意条。具体差异见 [内容编辑记录](content/EDITORIAL.md)。阅读通信桥和无效引用占位标记清理作用于构建输出，不回写原稿或内容副本。
...
设计依据为 `docs/design/` 中的原产品文档及补充的 part0–part2 文档，原文保持不变。
...
[发布验收记录](artifacts/verification.json)包含测试、生产检查、远端产物校验以及本轮 CloudBase 备份限制。
